...
const ServerSearch = ({ data }: ServerSearchProp) => {
    ...
    const onClick = ({ id, type }: { id: string, type: "channel" | "member" }) => {
        setOpen(false);
        if (type === 'member') {
            return router.push(`/server/${params?.serverId}/conversation`)
        }
        if (type === 'channel') {
    ...
    return (
        <>
            ...
            dark:hover:bg-zinc-700/50 transition">
                ...
                <p
                    className="font-se text-sm text-zinc-500 dark:text-zinc-400
                group-hover:text-zinc-600
                dark:group-hover:text-zinc-300 transition

                ">Search</p>
                <kbd
                    className="pointer-events-none inline-flex h-5 select-none items-center gap-1 rounded border bg-muted px-1.5 font-mono text-[10px] font-medium text-muted-foreground m-auto"
                >
